...
router.post('/', async(req, res) => {
    ...
    try{
        const update = User.updateOne({username: username}, { $inc:{games_won: 1, current_winstreak: 1}}).then(() => {
            ...
            const updatematch = Match.updateOne({player1: player1, plaeyr2: player2, timestamp: adapted_timestamp}, {winner: req.body.username}).then()
